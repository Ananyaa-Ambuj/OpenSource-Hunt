#!/usr/bin/env python3
"""Generator script: writes the complete create_issues.py with all issues."""
import json
...
# Write the complete create_issues.py
...
# Build the ISSUES list as Python code
lines = []
for issue in ISSUES:
    lines.append("ISSUES.append({")
    lines.append(f'    "title": {json.dumps(issue["title"])},')
    lines.append(f'    "body": {json.dumps(issue["body"])},')
    lines.append(f'    "labels": {json.dumps(issue["labels"])},')
    lines.append("})")
...
with open("create_issues.py", "w", encoding="utf-8") as f:
    f.write(full_content)

print(f"Written create_issues.py with {len(ISSUES)} issues")
